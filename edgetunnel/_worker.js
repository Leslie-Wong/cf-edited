				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							////LES EDIT START replace  "if (!url.searchParams.has('sub')) { // 本地生成订阅" to "const ECHLINK参数" 前
							if (!url.searchParams.has('sub')) { // 本地生成订阅
							...
							if(url.searchParams.has('sub') && config_JSON.优选订阅生成.SUB){
							...
							if(url.searchParams.has('best_cf_ip')){
									const BEST_CF_IP = url.searchParams.get('best_cf_ip').split(",");
									// return new Response('订阅转换后端异常：' + JSON.stringify(BEST_CF_IP) , { status: 200 });
									for (let CFIP of BEST_CF_IP ) {
											const [优选生成器IP数组, 优选生成器其他节点] = await 获取优选订阅生成器数据(CFIP);
											完整优选IP = 完整优选IP.concat(优选生成器IP数组);
											其他节点LINK += 优选生成器其他节点;
									}
							}
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								////LES EDIT START 在 "}).filter(item => item !== null).join('\n');" 前加入
								if(url.searchParams.has('vtype')){
	                                const VTYPES = url.searchParams.get('vtype').split(",");
	                                let data = [];
	                                for (let VTYPE of VTYPES ) {
	                                    data.push(`${VTYPE}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?security=tls&type=${config_JSON.传输协议 + ECHLINK参数}&host=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&path=${encodeURIComponent(作为优选订阅生成器 ? '/' : (config_JSON.随机路径 ? 随机路径(完整节点路径) : 完整节点路径)) + TLS分片参数}&encryption=none${config_JSON.跳过证书验证 ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(节点备注)}`);
	                                }
									return data.join('\n')
	                            }else{
